...
app.get('/season-points/:playerId/:tournamentId', async (req, res) => {
  ...
  const totalPoints = games.reduce((sum: number,  game: { points: number; }) => sum + game.points, 0);
...
app.get('/player-total-cost/:playerId/:tournamentId', async (req, res) => {
  ...
  const totalCost = games.reduce((sum: number, game: any) => sum + game.buyin + game.rebuys, 0);
...
app.get('player-total-rebuys/:playerId/:tournamentId', async (req, res) => {
  ...
  const totalRebuys = games.reduce((sum: Number, game: any) => sum + game.rebuys, 0)
  ...
  try {
    for (const playerId of players) {
      ...
      const game = await prisma.playerStats.create({
        data: {
          partyId: 1, // This should be replaced with actual partyId
          playerId,
          points: 0,
          buyin: 1,
          rebuys: 0,
...
app.post("/playerStats", async (req, res) => {
  try {
    const { partyId, playerId, points, rebuys, buyin , position,
      outAt} = req.body;
