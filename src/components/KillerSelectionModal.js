...
const KillerSelectionModal = ({ killer, games, currentlyPlayingPlayers, rebuyPlayerId, playerOutGame, handlePlayerKillSelection, }) => {
    ...
    return (_jsx(Dialog, { open: killer, onOpenChange: () => { }, children: _jsxs(DialogContent, { className: cn("bg-zinc-900", "border-2 border-blue-500", "backdrop-blur-md", "shadow-lg shadow-black/25", "max-w-[90vw] sm:max-w-[600px]", "p-6"), children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { style: {
                                textShadow: "2px 2px 10px 2px rgba(0, 0, 0, 0.3)",
                            }, className: cn("font-['DS-DIGI']", "text-3xl sm:text-4xl", "text-center", "text-white", "tracking-wider", "font-bold"), children: "Select the Killer" }), affectedPlayer && (_jsxs("div", { className: cn("text-center", "text-blue-200/90", "font-['DS-DIGI']", "text-xl", "leading-relaxed", "mt-4"), children: ["Who eliminated", " ", _jsx("span", { className: "font-bold", children: currentlyPlayingPlayers.find(p => p.id === affectedPlayer.playerId)?.name }), "?"] }))] }), _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8", children: availableKillers.map((player) => {
