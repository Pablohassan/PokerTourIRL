...
const testParty: PlayerPartyData[] = [
...
console.log(
  `Total pot calculation: ${testParty.length} players × 5€ = ${initialBuyins}€ initial buy-ins`
);
console.log(
  `                      ${totalRebuys} rebuys × 4€ = ${rebuyAmount}€ rebuys`
);
...
testParty.forEach((player) => {
  ...
  console.log(
    `Player ${player.playerId} (Position ${player.position}): ${
      gain >= 0 ? "+" : ""
    }${gain.toFixed(2)}€`
  );
});
...
console.log(
  `1st place (${firstPrizePct * 100}%): ${(totalPot * firstPrizePct).toFixed(
    2
  )}€`
);
console.log(
  `2nd place (${secondPrizePct * 100}%): ${(totalPot * secondPrizePct).toFixed(
    2
  )}€`
);
console.log(
  `3rd place (${thirdPrizePct * 100}%): ${(totalPot * thirdPrizePct).toFixed(
    2
  )}€`
);
console.log(
  `4th place (${fourthPrizePct * 100}%): ${(totalPot * fourthPrizePct).toFixed(
    2
  )}€`
);
...
console.log(
  `1st place: ${(totalPot * firstPrizePct).toFixed(
    2
  )}€ prize - ${winnerCost}€ cost = ${(
    totalPot * firstPrizePct -
    winnerCost
  ).toFixed(2)}€ gain`
);
console.log(
  `2nd place: ${(totalPot * secondPrizePct).toFixed(
    2
  )}€ prize - ${runnerUpCost}€ cost = ${(
    totalPot * secondPrizePct -
    runnerUpCost
  ).toFixed(2)}€ gain`
);
console.log(
  `3rd place: ${(totalPot * thirdPrizePct).toFixed(
    2
  )}€ prize - ${thirdCost}€ cost = ${(
    totalPot * thirdPrizePct -
    thirdCost
  ).toFixed(2)}€ gain`
);
console.log(
  `4th place: ${(totalPot * fourthPrizePct).toFixed(
    2
  )}€ prize - ${fourthCost}€ cost = ${(
    totalPot * fourthPrizePct -
    fourthCost
  ).toFixed(2)}€ gain`
);

export default function runTests(): TestResults {
