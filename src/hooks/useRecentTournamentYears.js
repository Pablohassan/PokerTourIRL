...
const uniqueSortedDesc = (years) => {
    return Array.from(new Set(years)).sort((a, b) => b - a);
};
export const useRecentTournamentYears = () => {
        ...
        try {
            ...
            if (!fetchedYears.includes(currentYear)) {
                try {
                    ...
                    fetchedYears = fetchedYears.concat(Number.isFinite(createdYear) ? createdYear : currentYear);
                }
                catch (createError) {
                    try {
            ...
            setDefaultYear(recentYears.includes(currentYear) ? currentYear : recentYears[0] ?? null);
        }
        catch (fetchError) {
